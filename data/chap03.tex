...
\begin{equation}
  ...
  \end{array}
\end{equation}
得到的~$Support_\mathbb{D}(d_3')$~仍为~$D_3(0.5,-1)$，与选择二范数最小点~$I$~所代表的方向达到相同的结果。 
...
\begin{equation}
  ...
  \end{array}
\end{equation}
事实上，选择二范数最小点的坐标代表方向与该垂直方向相同，因此结果一致。
图~\ref{fig:gjk:example:nonIntersection:step4}~中，
$\overrightarrow{D_3'D_2'}$~垂直并指向阴影部分面积方向为~$\overrightarrow{D_3'D_2'_p}=\overrightarrow{D_3'D_2'}
\times \overrightarrow{D_3'D_1'} \times \overrightarrow{D_3'D_2'} = 
(1.5, 0)*(-3) + 2.25*(2,2) =(0, 4.5)$
$\overrightarrow{D_3'D_1'}$~垂直并指向阴影部分面积方向为~$\overrightarrow{D_3'D_1'}
\times \overrightarrow{D_3'D_2'} \times \overrightarrow{D_3'D_1'} =
(-2,-2)*(-3) + 8*(-1.5, 0) = (-6, 6)$
还都看一遍这个例子 走网页那个代码。TODO
$D_3'D_2'_p \cdot D_3'O = 4.5 > 0$且$D_3'D_1'_p \cdot D_3'O = >0$<++>
