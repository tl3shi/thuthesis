...
\begin{figure}[!ht]
  \centering
    \includegraphics[width=4.5in]{TriangleTriangleTest.eps}
...
针对情况(1)，共面的两个三角形求交可以通过两个三角形中三条线段两两判定是否相交最多9次线段线段求交判定可得，或者通过我们在文献~\onlinecite{linjianli2014}~中提出的方法进行，该方法对线段三角形的位置做了详细的分类可通过不超过~6~次线段线段求交判定；
...
\begin{figure}[htbp]
  \centering
    \includegraphics[width=3.5in]{TriangleTriangleTest2.eps}
...
完整的算法如算法~\ref{alg:triangles:intersection}~所示。
...
本章实验所采用的模型如表~\ref{tab:exp:models}~所示，模型点集大小从~2.5k~到~225k~不等，三角面片的数量从近~5k~到~264k~不等，除了第2章的部分模型外，本章还用到了~CollDet~库所提供的部分模型。
...
\begin{figure}[!ht] 
\centering
\subcaptionbox{包围体构造时间对比\label{chart:exps:bv:ct}}
{
    \includegraphics[width=0.48\textwidth,page=1]{bvprebench.pdf}
}
\subcaptionbox{包围体命中率对比\label{chart:exps:bv:hitrate}}
{  
   \includegraphics[width=0.48\textwidth,page=3]{bvprebench.pdf}
}\\
\subcaptionbox{包围体初始化时间对比\label{chart:exps:bv:init}}
{  
   \includegraphics[width=0.48\textwidth,page=6]{bvprebench.pdf}
}
\subcaptionbox{碰撞检测过程时间对比\label{chart:exps:bv:cd}}
{  
   \includegraphics[width=0.48\textwidth,page=8]{bvprebench.pdf}
}
\caption{不同包围体实验结果对比}
\label{chart:exps:bv:comp}
\end{figure}

图~\ref{chart:exps:bv:comp}~为在2G内存的限制下不同包围体应用于静止场景的碰撞检测实验结果，其中~CT~表示单个模型包围体的平均构造时间，$r$~为包围体的命中率，Box、Sphere~和~CHull~分别是~AABB~包围体、Ritter~\cite{Ritter1990}算法计算出的包围球和~CGAL~计算的凸包，$k$-CBP~和$k$-DOP~中的~$k$~值为16，且~$k$-DOP~构造过程中在此实验中没有计算其顶点，场景中随机放置的模型数量~Bunny、Apple~和~HappyBuddha~各为100个，Dinosaur~和~Hand~各50个，35个~Dragon~和25~个Alice。
从中可得，从包围体的构造时间上将基本满足：$\textnormal{凸包} > k\textnormal{-CBP} > k\textnormal{-DOP} > \textnormal{Sphere} \approx  \textnormal{Box}$，
...
在此实验中，一方面，命中率高的包围体可以减少不必要的真实模型之间的碰撞检测，并因此提高整体算法的效率；而另一方面命中率高的包围体往往构造时间会更复杂，且包围体间的相交检测也更复杂。
图~\ref{chart:exps:bv:init}~中的~init~时间代表初始化时间即包围体构造时间加上各个包围体相交检测后对相关模型进行构造~AABB~树的总时间，
因为~Sphere~的命中率最低，因此需要构造AABB树的模型数量较多，因此初始化时间耗时更多，相应的因为凸包构造时间较长，虽然其命中率高，但初始化时间也不是最少的，而~$k$-DOP~和~$k$-CBP~相比，虽然~$k$-CBP~命中率稍高但因其需要计算凸包围的的顶点因此构造时间稍长而~$k$-DOP~在本实验中不需要计算顶点，因此综合来看二者初始化时间相差不大。
...
\subsection{静止场景中与基于~$k$-DOP~树算法对比}
...
图~\ref{chart:exps:kdop:kcbp:bunny}~为本文的两种算法和文献~\onlinecite{abenchmarking2007}~中基于~$k$-DOP~的实现的实验结果的曲线展示图，
...
\subcaptionbox{Dinosaur(80554个三角网格)\label{fig:exp:static:dinosaur}}
{  
   \includegraphics[width=\fourgraphicswidth\textwidth, page=6]{staticcd.pdf}
}
\caption{静止场景下本文算法与基于~$k$-DOP~树实验结果对比（$k=24$）}
...
从中可得，在静态碰撞检测实验中，本文算法与基于$k$-DOP~树的算法相比，在初始化环境过程中占用更少的时间，而在碰撞检测过程中针对不同的模型可能有不同的结果。从整体来看，本文算法优于基于~$k$-DOP~树的算法，平均而言耗时是基于~$k$-DOP~树算法的0.8 $\sim$ 3.2倍。
...
\subcaptionbox{Dinosaur(80554个三角网格)\label{fig:exp:static:k46:dinosaur}}
{  
...
对于更大的~$k$~值，构造层次结构的$k$-DOP~树时会耗费更多的时间和存储空间，在模型较大且模型数量较多时，以文献~\onlinecite{abenchmarking2007}~中最大的~$k$值为46~为例，32位应用程序已经满足不了构造~70~个~Dinosaur~模型~的~$k$-DOP~树，且构造时间也更久，图~\ref{fig:chart:exps:kdop:kcbp:k46}~为~$k$-DOP~实现中的最大值~$k=46$~的实验结果。

更大的~$k$~值不一定能够使碰撞检测的效率提高，这与具体的碰撞检测环境有关，以~Bunny~模型和~HappyBuddha~模型为例，如图~\ref{fig:chart:exp:kdop:kcbp:k24:k46}~为~$k=24$~和~$k=46$~的结果对比，
...
\subcaptionbox{HappyBuddha(50000个三角网格)\label{fig:exp:static:k24:k46:happybuddha}}
{  
    \includegraphics[width=\fourgraphicswidth\textwidth, page=18]{staticcd.pdf}
}
...
表~\ref{tab:exper:kcbp:kdop:init:k24}~为本文算法和~$k$-DOP~算法均采用~CollDet~实现中~$k=24$~默认值的初始化时间对比，init($k$-CBP)~所代表的时间表示构造~$k$-CBP~及模型
...
横坐标表示其中一个模型运动的步数~$frames$，纵坐标表示随机运动~$frames$~步碰撞检测所耗费的时间，实心圆所代表曲线为前文提出的基于~GJK~算法实现，菱形所代表的曲线为基于~$k$-DOP~实现，另外的为前文提出的基于~AABB~树实现。
...
\subcaptionbox{Dinosaur(80554个三角网格)\label{fig:exp:dynamic:dinosaur}}
{  
...
\caption{运动场景下本文算法与基于~$k$-DOP~树实验结果对比（$k=24,n=2$）}
...
随着三角网格增多，三种算法所耗费时间均有所增加，总体而言，基于~$k$-DOP~的算法比较稳定，而本文算法随着模型不同有一定波动。
...
%图~\ref{fig:exp:dynamic:budda}~所示的~Budda~模型在~s=700$~时，均耗时200多毫秒，因为此时$k$-CBP~相交对数为~DP($k$-CBP)$=10$，且实际模型相交对数为5，且在这5次模型真实相交过程中，调用单独包围盒和旋转包围盒碰撞检测算法最大达46044次，且三角网格相交测试数量也达到8040次，因此总体耗时很久，深究原因发现~Budda~模型的底部包含有很多极小的共面三角形，导致很多这些极小的共面三角形与其他三角形发生多次碰撞检测，这表明本文算法存在一定鲁棒性问题。
...
\subcaptionbox{Dinosaur(80554个三角网格)\label{fig:exp:dynamic:m10:dinosaur}}
{  
...
从中可看出，当模型三角网格数量较少时，本文基于~$k$-CBP~结合~AABB~树的算法耗时最多，而随着三角网格数量增大，此算法和基于~$k$-DOP~树的算法耗时相差不大，但都多余本文基于$k$-CBP~结合~GJK~的算法。
整体来看，在运动场景的碰撞检测环境中，本文基于$k$-CBP~结合~GJK~的算法在一定程度上有一定优势，从实验结果来看平均而言是基于~$k$-DOP~树算法的0.8 $\sim$ 5.6倍，而基于$k$-DOP~算法相对较稳定，但其初始化时间很长。
...
\subcaptionbox{HappyBuddha(50000个三角网格)\label{fig:exp:static:k24:k46:happybuddha:dynamic}}
{  
    \includegraphics[width=\fourgraphicswidth\textwidth, page=18]{dynamiccd.pdf}
}
...
实验结果表明本文基于~$k$-CBP~的算法在运动场景中二者相差不大，而另外两种算法在~$k=24$~时碰撞检测所耗费的时间更少。因此在实际应用环境中应该根据具体的碰撞检测环境
选择不同的~$k$~值。
...
实验结果表明平均而言本文算法初始化过程提高了至少8倍，在静止场景下，本文基于~$k$-CBP~结合GJK的算法是基于~$k$-DOP~树算法的~0.8 $\sim$ 3.2~倍，而运动场景下是基于~$k$-DOP~树算法的~0.8 $\sim$ 5.6~倍，因此本文算法能够有效加速碰撞检测过程。
