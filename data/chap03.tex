...
具体算法如算法~\ref{alg:aabbtree:build}~所示，该算法是一个递归算法，%输入为模型的三角网格数组~$primitives$~和对应递归调用的上下限~$first,last$，
...
算法~\ref{alg:gjk}~第~12~行~\textproc{CheckContainUpdate}~子过称需检测~$\bm{S}$~是否包含原点，必要时进行规约并获取下一次迭代方向，以上两个例子已经对二维情况进行分析，
...
算法~\ref{alg:triangles:intersection}~第~10~行~在方法~\textproc{EdgeEdgeTest}~进行边边测试子过程中，可通过点与有向线段的位置关系确定，如线段两个端点都在另外一个有向线段的一边说明两线段不相交，反之则相交，不需要求解出具体的交点坐标。
...
\Function{TransformBox}{$box, \bm{M}$}
    \State $vertices \gets \Call{GetAABBVertices}{box}$ \Comment{获取~AABB~的8个顶点}
...
最后分别从静止场景和运动场景中与文献~\onlinecite{abenchmarking2007}中的基于~$k$-DOP~树的算法对比。
...
本章实验所采用的模型如表~\ref{tab:exp:models}~所示，模型点集大小从~2.5k~到~225k~不等，三角面片的数量从近~5k~到~264k~不等，除了第2章的部分模型外，本章还用到了~CollDet~库所提供的部分模型\footnote{实验中用到的~HappyBuddha、Dragon~和~Hand~等模型可从~http://cgvr.cs.uni-bremen.de/research/colldet\_benchmark/下载。}。
...
因而命中率比包围盒高，表中所示~16-CBP~比包围盒的命中率高~30\% $\sim$ 40\%，能尽早排除更多本不相交的模型从而节省碰撞检测总时间，提高算法效率。该部分工作已经发表，详细内容见参考文献~\onlinecite{tanglei2014}。
...
\subcaptionbox{碰撞检测初始化时间对比\label{chart:exps:bv:init}}
{  
...
图~\ref{chart:exps:kdop:kcbp:bunny}~为本文的两种算法和文献~\onlinecite{abenchmarking2007}~中基于~$k$-DOP~树实现的实验结果的曲线展示图，
...
对于更大的~$k$~值，构造层次结构的$k$-DOP~树时会耗费更多的时间和存储空间，在模型较大且模型数量较多时，以文献~\onlinecite{abenchmarking2007}~中最大的~$k$值为46~为例，32位应用程序已经满足不了构造~70~个~Dinosaur~模型~的~$k$-DOP~树，且构造时间也更久，图~\ref{fig:chart:exps:kdop:kcbp:k46}~为基于~$k$-DOP~树算法实现中的最大值~$k=46$~的实验结果。
...
表~\ref{tab:exper:kcbp:kdop:init:k24}~为本文算法和基于~$k$-DOP~树算法均采用~CollDet~实现中~$k=24$~默认值的初始化时间对比，init($k$-CBP)~所代表的时间表示构造~$k$-CBP~及模型
...
{  
   \includegraphics[width=0.42\textwidth,page=7]{dynamiccd.pdf}
}
%\subcaptionbox{Apple(8040个三角网格)\label{fig:exp:dynamic:m10:apple}}
%{  
%    \includegraphics[width=0.42\textwidth, page=8]{dynamiccd.pdf}
%}\linebreak %强制换行
\subcaptionbox{HappyBuddha(50000个三角网格)\label{fig:exp:dynamic:m10:happyBuddha}}
{  
   \includegraphics[width=0.42\textwidth, page=14]{dynamiccd.pdf}
}\linebreak %强制换行
\subcaptionbox{Dinosaur(80554个三角网格)\label{fig:exp:dynamic:m10:dinosaur}}
{  
   \includegraphics[width=0.42\textwidth, page=10]{dynamiccd.pdf}
}
\subcaptionbox{Hand(128314个三角网格)\label{fig:exp:dynamic:m10:hand}}
{  
   \includegraphics[width=0.42\textwidth, page=16]{dynamiccd.pdf}
}
...
整体来看，在运动场景的碰撞检测环境中，本文基于$k$-CBP~结合~GJK~的算法在一定程度上有一定优势，从实验结果来看平均而言效率是基于~$k$-DOP~树算法的0.8 $\sim$ 5.6倍。%，而基于$k$-DOP~算法相对较稳定，但其初始化时间很长。
\begin{figure}[!ht] 
\centering
\subcaptionbox{Bunny(4968个三角网格)\label{fig:exp:static:k24:k46:bunny:dynamic}}
{  
   \includegraphics[width=0.42\textwidth,page=11]{dynamiccd.pdf}
}
\subcaptionbox{HappyBuddha(50000个三角网格)\label{fig:exp:static:k24:k46:happybuddha:dynamic}}
{  
    \includegraphics[width=0.42\textwidth, page=18]{dynamiccd.pdf}
}
