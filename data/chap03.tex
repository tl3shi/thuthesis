...
为~2~个~Bunny~模型在随机运动~$moves$~步和~$k$-DOP~算法的比较结果，当真实模型相交数量较多时，三种算法所耗费的时间也更多，总体来看本文算法是基于~$k$-DOP~算法耗时的~0.8 $\sim$ 3.75~倍。
...
%\multirow{2}{*}{$n$} & \multirow{2}{*}{$n$(Box)} & CT($k$-DOP) &  CT($k$-CBP) & DT($k$-DOP) & DT($k$-CBP) & DT($k$-CBP) \\%& \multirow{2}{*}{DP(Model)} \\ 
%                     &                         &  (ms)        & (ms)         & (ms)        & $(AABB)$~(ms) & $(GJK)$~(ms)       \\
\multirow{2}{*}{$moves$} & DT($k$-CBP)(AABB) & DT($k$-DOP) & DT($k$-CBP)(GJK) & DP($k$-CBP) & DP(Model) \\
                         & (ms)              &  (ms)       & (ms)           & (对)&(对) \\
\midrule[1.0pt]
