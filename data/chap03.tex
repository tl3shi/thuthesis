...
另四边形~$DEFG$~为~$\mathbb{A}$~
三角形~$ABC$~为~$\mathbb{B}$~

第一步 $\bm{d_1} = (1,0)$

\begin{equation}
  Support_\mathbb{D}(d_1)
  \begin{array}{ll}
    & = Support_\mathbb{A}(\bm{d_1}) - Support_\mathbb{B}(-\bm{d_1}) \\
    & = E(-2.5, 2) - A(-3,1) \\
    & = D_1(-0.5, 1)
  \end{array}
...
\begin{figure}[htbp]
\centering
\subcaptionbox{第一步\label{fig:gjk:example:intersection:step1}}{
  \includegraphics[width=.48\textwidth]{gjkExampleIntersectionStep1.tikz}
} 
\subcaptionbox{第二步\label{fig:gjk:example:nonIntersection:step2}}{
  \includegraphics[width=.48\textwidth]{gjkExampleIntersectionStep2.tikz}
}
\subcaptionbox{第三步\label{fig:gjk:example:nonIntersection:step3}}{
  \includegraphics[width=.48\textwidth]{gjkExampleIntersectionStep3.tikz}
}
\caption{二维~GJK~算法相交迭代示例}
\label{fig:gjk:example:2d:intersection:iterator}
\end{figure}

第二步 取 $\bm{d_2}=-\bm{D_1}=(-0.5, -1)$，

\begin{equation}
  Support_\mathbb{D}(d_2)
  \begin{array}{ll}
    & = Support_\mathbb{A}(\bm{d_2}) - Support_\mathbb{B}(-\bm{d_2}) \\
    & = D(-4, 2) - B(-1, 3) \\
    & = D_2(-3, -1)
  \end{array}
...
第三步，simplex 为 $D_1, D_2$，
取最近的点 $I(-4/13, 7/13)$所代表方向，$\bm{d}_3$
取 $\bm{d_3}=-\bm{I}=(4/13, -7/13)$，

\begin{equation}
  Support_\mathbb{D}(d_3)
  \begin{array}{ll}
    & = Support_\mathbb{A}(\bm{d_3}) - Support_\mathbb{B}(-\bm{d_3}) \\
    & = E(-2.5, 2) - C(-3, 3) \\
    & = D_3(0.5, -1)
  \end{array}
