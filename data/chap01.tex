...
凸包围体能用于在原始模型之间的相关计算（遮挡测试、相交测试等）之前进行预处理判断和剪枝的理论基础正是基于此。通常情况下（无特别强调，本文不考虑包围体比原始模型还复杂的情况），凸包围体之间的计算比其包含的原始模型计算更简单，当凸包围体之间没有相交或遮挡时，原始模型一定没有相交或遮挡。
...
$k$-DOP~之间的相交测试如算法~\ref{algo:kdop:overlaptest}~所示\cite{ericson2005real}，节点的~$min[i]$~和~$max[i]$~表示分别沿着第~$i$~个方向最小和最大的投影值，与~AABB~包围体的相交测试相似，不同点在于~AABB~包围体是针对~$x,y,z$~共3个轴方向进行判断是否有交叉重叠，
...
\indent
\item \textbf{其他}
还有如图~\ref{fig:subfig-bv-cylinder}~所示的圆柱形\cite{Schomer2000Smallest}、图~\ref{fig:subfig-bvcone}~所示的圆锥形\cite{held1997erit}和如图~\ref{fig:subfig-bv-ellipse}~所示的椭球形\cite{Wang2004Efficient,hebing2009}等包围体。
\end{inparaenum}
...
\onlinecite{madera2009hybrid,real2001,vogiannou2010enhancing,virtual2007,chang2010efficient,tang2010fast,zhigang2010efficient,robot2014,game3d2010} 等。
...
碰撞检测算法是许多应用的基础，例如在~3D~游戏\cite{game3d2010}，物理仿真\cite{real2001}，机器人\cite{robot2014}，虚拟手术\cite{virtual2007}和虚拟现实\cite{Youn1993Realtime}等领域中。
本文将结合传统的包围体树和提出的~$k$-CBP~应用于碰撞检测算法，通过更加紧致的~$k$-CBP~对碰撞检测环境中的模型进行剪枝来提高算法效率，
本节将就碰撞检测问题的分类和基于包围体树的算法进行阐述。
...
另外一类就是层次结构包围体（Bounding Volume Hierarchies，简称~BVH，也称包围体树），如在第~\ref{sec:convex-bv}~节中提到的~AABB~，~OBB~等包围体树。
...
根据碰撞检测的模拟环境划分，又可以分为成对（Pair Processing）碰撞检测和多体（Nbody Processing）碰撞检测，刚体和柔性可变形模型的碰撞检测，静止和运动场景的碰撞检测以及离散和连续的碰撞检测\cite{lin1998collision}。
...
文献~\onlinecite{Kockara2007Collision}将常见的碰撞检测算法分为粗粒度检测和细粒度检测两个阶段，并对各阶段各种算法的特征进行较为详细的分析，文中指出基于~BVH~的方法是碰撞检测中更为通用的方法。
...
遍历过程中，若检测到任意两个叶子节点包含的原生几何相交时，即可终止遍历，相应的原始模型发生碰撞。
...
第一章首先介绍了当前各种凸包围体的特征及应用，然后介绍了碰撞检测问题的背景和本文研究的基于凸包围体的碰撞检测算法。
