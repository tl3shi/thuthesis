   ...
   \begin{block}{并行可行性}
    ...
    \frametitle{GLSL实现}
        ...
        \note{
          ...
          \scriptsize
          另外一种是乒乓技术，乒乓技术是需要把前一次运算结果传递给下一次运算用来作为后继运算的输。算法将输入点分成 x 份，每次只处理一份，在当前渲染过程中，找出在该批点集中沿着所有法向投影值最大的点，当处理下批点集即下一次渲染时，将与上次局部最大投影值进行比较，若有新的投影最大值就更新，如此反复 x 次，当所有点都被处理完毕后得到沿着所有法向投影最大值的点即切点。所以这个算法对k不是很敏感，因为k较小时，GPU一次总能处理k个方向，这点从后面的实验结果也能看出。
        }
   }
